...
from core.models import DECISION_HEALTHY, DECISION_UNKNOWN
from core.parse.runner_log import diagnose_runner
from doors.cli import EXIT_FAILED, EXIT_OK, render_terminal, watch_nextflow
...
@pytest.mark.parametrize(
    ("name", "rule"), [("low_depth.vcf", "QC-VCF-01"), ("high_missingness.vcf", "QC-VCF-02")]
)
def test_vcf_rules_depth_and_missingness_have_receipts(name, rule):
    ...
    assert hashlib.sha256((FIX / "vcf" / name).read_bytes()).hexdigest() == fr.version.removeprefix(
        "sha256:"
    )
...
def test_snakemake_each_signature_and_unknown_have_expected_receipts():
    cases = {
        "missing_input": "SM-INPUT-001",
        "ambiguous_rule": "SM-AMBIG-002",
        "unknown_target": "SM-TARGET-003",
        "wildcards": "SM-WILDCARD-004",
        "conda": "SM-CONDA-005",
        "job_failed": "SM-JOB-006",
        "incomplete": "SM-INCOMPLETE-007",
        "cycle": "SM-CYCLE-008",
    }
...
def test_snakemake_one_shot_and_eof_diagnosis_are_equivalent():
    ...
    assert [(f.id, f.details) for f in one_shot.findings] == [
        (f.id, f.details) for f in at_eof.findings
    ]


def test_cromwell_signature_fixtures_and_unknown():
    cases = {
        "failed_call": "CW-CALL-001",
        "shard_retry": "CW-SHARD-002",
        "backend": "CW-BACKEND-003",
        "localization": "CW-LOCALIZE-004",
        "capture": "CW-CAPTURE-005",
    }
    for name, rule in cases.items():
...
def test_watcher_detects_midrun_oom_but_holds_partial_line(capsys):
    ...
    code = watch_nextflow(
        str(FIX / "logs" / "growing" / "truncated.part"),
        0.01,
...
def test_core_has_no_network_imports_for_new_stations():
    for p in [
        ROOT / "core/parse/vcf.py",
        ROOT / "core/rules/vcf_rules.py",
        ROOT / "core/parse/runner_log.py",
    ]:
        text = p.read_text().lower()
        assert not any(
            word in text
            for word in ("requests", "httpx", "urllib", "socket", "openai", "anthropic")
        )
