...
from ngs_agent.agentic.bioinformatics.rnaseq.pipeline import RNASeqPipelineAdapter
from ngs_agent.agentic.config.settings import load_settings
...
from ngs_agent.agentic.observability.logging import configure_logging
from ngs_agent.agentic.tools.registry import create_default_registry


class NaturalLanguageGroup(TyperGroup):
    """Group that routes unrecognized leading tokens to the `analyze` command.

    This keeps `ngs run rnaseq ...` and other subcommands dispatching normally
    while still allowing free-form prompts like `ngs analyze this experiment`.
    A greedy variadic argument on the root callback would swallow subcommand
    names, so natural-language routing must happen here instead.
    """
    ...
    def parse_args(self, ctx: click.Context, args: list[str]) -> list[str]:
        for index, token in enumerate(args):
            ...
            if token not in self.commands:
                args = [*args[:index], "analyze", *args[index:]]
            break
...
def root(
    ...
    """Agentic NGS CLI. Pass a natural-language prompt or use a subcommand."""
...
def run_rnaseq(
    ...
    working_directory: Annotated[Path | None, typer.Option("--working-directory", help="Experiment root directory (defaults to current directory)")] = None,
