...
from ngs_agent.agentic.tools.base import Tool, ToolContext
...
def _fastqc_stem(path: Path) -> str:
    """Reproduce FastQC's report naming: it strips known fastq extensions.

    ``Path.stem`` alone is wrong for double extensions, e.g. ``sample.fastq.gz``
    yields ``sample.fastq`` while FastQC writes ``sample_fastqc.html``.
    """
    name = path.name
    for ext in (".fastq.gz", ".fq.gz", ".fastq", ".fq", ".gz", ".bam", ".sam"):
...
class FastQCTool(BioinformaticsTool):
    ...
    def execute(self, payload: FastQCInput, context: ToolContext) -> FastQCOutput:
        ...
        html_reports = [str(payload.output_dir / f"{_fastqc_stem(read)}_fastqc.html") for read in payload.reads]
        zip_reports = [str(payload.output_dir / f"{_fastqc_stem(read)}_fastqc.zip") for read in payload.reads]
        return FastQCOutput(
            command=command,
            stdout=stdout,
            stderr=stderr,
...
class TrimmomaticTool(BioinformaticsTool):
    ...
    def _decide_trim_params(self, payload: TrimmomaticInput) -> tuple[dict[str, Any], str, float]:
        ...
        if api_key and fastqc_summary:
            try:
                # Lazy import: anthropic is an optional [llm] extra dependency.
...
class HISAT2Tool(BioinformaticsTool):
    ...
    def execute(self, payload: HISAT2Input, context: ToolContext) -> HISAT2Output:
        ...
        # Parse mapping rate from hisat2's stderr summary (e.g. "97.35% overall alignment rate").
        mapping_rate = "unknown"
        for line in stderr.splitlines():
            if "overall alignment rate" in line:
                mapping_rate = line.strip().split("%")[0].strip() + "%"
                break

        # Convert SAM -> sorted BAM and index it so bam_path/bam_index actually exist.
        if returncode == 0:
            sort_cmd = ["samtools", "sort", "-@", str(payload.threads), "-o", str(payload.output_bam), str(sam_path)]
            sort_stdout, sort_stderr, sort_rc, _ = self._run(sort_cmd, context, "samtools sort")
            stdout += sort_stdout
            stderr += sort_stderr
            if sort_rc == 0:
                sam_path.unlink(missing_ok=True)
                index_cmd = ["samtools", "index", str(payload.output_bam)]
                idx_stdout, idx_stderr, idx_rc, _ = self._run(index_cmd, context, "samtools index")
                stdout += idx_stdout
                stderr += idx_stderr
                returncode = idx_rc
            else:
...
class DESeq2Tool(BioinformaticsTool):
    ...
    def execute(self, payload: DESeq2Input, context: ToolContext) -> DESeq2Output:
        ...
        command = [
            "Rscript",
            str(Path(__file__).resolve().parents[2] / "scripts" / "deseq2_analysis.R"),
...
class GOEnrichmentTool(BioinformaticsTool):
    ...
    def execute(self, payload: GOEnrichmentInput, context: ToolContext) -> GOEnrichmentOutput:
        ...
        command = [
            "Rscript",
            str(Path(__file__).resolve().parents[2] / "scripts" / "enrichment_analysis.R"),
