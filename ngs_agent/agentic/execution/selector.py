...
from ngs_agent.agentic.execution.backends.native_backend import NativeBackend
    ...
    def select(self, preference: BackendPreference) -> SelectedBackend:
        ...
        if preference == "docker":
            ...
            if shutil.which("docker") is not None:
                logger.warning(
                    "Docker binary is installed but the daemon is unreachable; "
                    "falling back to native backend."
                )
                return SelectedBackend(
                    self._native,
                    "Fallback: docker requested but daemon unreachable; using native backend",
                )
            raise RuntimeError(
