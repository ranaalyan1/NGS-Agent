...
class AgentOrchestrator:
    def __init__(
        ...
        self.permission_policy = PermissionPolicy(
            require_confirmation_for={
                SafetyLevel(level) for level in settings.require_confirmation_for
            },
    ...
    def _render_plan(self, plan: Plan) -> None:
        ...
        for step in plan.steps:
            table.add_row(
                ...
                str(step.estimated_duration_minutes),
        ...
        self.console.print(
            Panel(
                ...
                f"[bold]Parallelism:[/bold] up to {plan.max_parallel_steps} concurrent steps\n"
    ...
    def _default_confirm(self, prompt: str) -> bool:
        return self.console.input(
            f"[bold yellow]{prompt} [/bold yellow][y/N] "
    ...
    def run(
        ...
        if not os.access(working_directory, os.R_OK | os.W_OK):
            raise PermissionError(
                f"Insufficient permissions for working directory: {working_directory}"
            )
        ...
        plan = self.planner.create_plan(
            objective=objective, workflow=workflow, context=context, dry_run=dry_run
        )
        self._render_plan(plan)

        if dry_run:
            self.console.print(
                "[green]Dry-run mode enabled. No execution will be performed.[/green]"
            )
            self.checkpoints.save(
                run_id, {"stage": "dry-run", "plan": plan.model_dump(mode="json")}
            )
            return {
                "run_id": run_id,
                "status": "dry-run",
        ...
        if not confirm(f"Execute plan '{plan.title}' with {len(plan.steps)} steps?"):
            ...
            self.checkpoints.save(
                run_id, {"stage": "cancelled", "plan": plan.model_dump(mode="json")}
            )
        ...
        # Write the provenance manifest next to the report.
        manifest_jsonl = manifest.write_jsonl(self.artifact_store.root / "manifest.jsonl")
        manifest.write_summary(self.artifact_store.root / "manifest.json")
        ...
        selected_backend = self.backend_selector.select(self.settings.backend_preference)
        verification: VerificationReport = self.verifier.verify(
            plan,
            execution_state.outcomes,
            search_roots=[self.artifact_store.root, working_directory / "results"],
        )
        self.checkpoints.save(
            run_id,
            {
        ...
        report: ReportBundle = self.reporter.generate(
            ...
            execution_state.outcomes,
            run_id=run_id,
            backend_name=selected_backend.backend.name,
            manifest=manifest,
        )
        # Final verification pass now that the manifest exists on disk; the
        # report itself is checked explicitly below.
        verification = self.verifier.verify(
            plan,
            execution_state.outcomes,
            search_roots=[self.artifact_store.root, working_directory / "results"],
        )
        if not Path(report.html_report).exists():
            logger.error(
                "Reporter did not produce the expected HTML report at %s", report.html_report
            )
        self.checkpoints.save(
            run_id,
            {
        ...
        self.console.print(f"[bold green]Provenance manifest:[/bold green] {manifest_jsonl}")

        status = "complete" if verification.passed else "completed-with-warnings"
        if execution_state.failure is not None:
            status = "failed"
        return {
            "run_id": run_id,
            "status": status,
            ...
            "report": report.model_dump(),
            "outcomes": [outcome.model_dump() for outcome in execution_state.outcomes],
            "wall_clock_seconds": execution_state.wall_clock_seconds,
            "max_observed_parallelism": execution_state.max_observed_parallelism,
