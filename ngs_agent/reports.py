...
def generate_html_report(
    ...
    # Build the model-consultation section. Narrative only: it carries no
    # classification, and the banner says so in terms a reader cannot miss.
    debate_section_html = ""
    if debates:
        debate_section_html = (
            "<h2>Model Consultation (narrative only)</h2>"
            '<div class="debate-section" style="border-left: 4px solid #b58900;">'
            "<strong>Not a classification.</strong> The text below was generated by language models "
            "from the input annotations alone. It is not evidence, it cites no evidence ledger, and "
            "it did not contribute to any ACMG/AMP determination. For an evidence-backed "
            "classification run <code>ngsagent review</code>. Classification language and ACMG "
            "criterion codes emitted by a model are stripped and reported as boundary violations."
            "</div>"
        )
        for d in debates:
            opinions_html = ""
            for op in d.opinions:
                ...
                if op.redactions:
                    redaction_html = (
                        '<p style="margin: 0.25rem 0 0 0; color: #b58900; font-size: 0.85rem;">'
                        f"Removed by guardrail: {html_escape(', '.join(op.redactions))}</p>"
                    )
                opinions_html += f"""
                <div class="opinion-box">
                  <strong>{html_escape(op.persona)}</strong>
                  <p style="margin: 0.25rem 0 0 0; color: var(--text-muted); font-size: 0.9rem;">{html_escape(op.reasoning)}</p>
                  {redaction_html}
                </div>
                """
            summary = d.summary
            checks_html = "".join(f"<li>{html_escape(item)}</li>" for item in summary.evidence_a_reviewer_should_check)
            violations_html = ""
            if d.boundary_violations:
                items = "".join(f"<li>{html_escape(item)}</li>" for item in d.boundary_violations)
                violations_html = (
                    '<p style="color: #b58900;"><strong>Boundary violations detected:</strong></p>'
                    f"<ul>{items}</ul>"
                )
            debate_section_html += f"""
            <div class="debate-section">
              <h3>{html_escape(d.variant.gene)} ({html_escape(str(d.variant.chrom))}:{d.variant.pos} {html_escape(d.variant.ref)}&gt;{html_escape(d.variant.alt)})</h3>
              {opinions_html}
              <div style="margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid var(--border);">
                <strong>What a reviewer should check:</strong>
                <ul>{checks_html}</ul>
                {violations_html}
                <p style="color: var(--text-muted); font-size: 0.85rem;">{html_escape(d.disclaimer)}</p>
              </div>
            </div>
            """
